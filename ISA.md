...
### Auth & Accounts
- [DEFERRED-VERIFY] ISC-1: `POST /api/auth/signup` with valid email+password creates an account row and returns 201.
...
- [DEFERRED-VERIFY] ISC-9: An unauthenticated request to `GET /api/monitors` returns 401.
...
### Monitor CRUD
- [DEFERRED-VERIFY] ISC-15: `POST /api/monitors` with `type: http`, a valid `url`, and `interval_seconds` creates a monitor row scoped to the authenticated account.
...
- [DEFERRED-VERIFY] ISC-22: `GET /api/monitors` returns only monitors owned by the authenticated account, newest first.
...
- [DEFERRED-VERIFY] ISC-25: `DELETE /api/monitors/:id` removes the monitor and all its check-history rows (cascade).
...
- [ ] ISC-36: A ping monitor check records `up` when the host responds and `down` with `failure_reason: unreachable` when it does not, within a 5-second timeout.
- [ ] ISC-37: A TCP monitor check records `up` when the configured port accepts a connection and `down` otherwise.
...
### Status Pages
- [ ] ISC-63: `GET /status/:slug` renders a public page (no auth required) showing the monitor's current status (`up`/`down`) and name, when the owner has published it.
- [ ] ISC-64: A monitor not marked `published: true` is not reachable via any `/status/:slug` URL — request returns 404.
...
- [ ] ISC-68: `PATCH /api/monitors/:id` can set/change the public `slug` for a monitor; slug collisions across accounts return 409.
- [ ] ISC-69: The status page never exposes the owning account's email, internal monitor ID, or any other monitor belonging to the same account unless that monitor is also independently published.
...
### Dashboard UI
- [ ] ISC-72: `/dashboard` (authenticated) lists all monitors for the logged-in account with current status badge (up/down/paused).
- [ ] ISC-73: Each monitor row shows the most recent response time and last-checked timestamp.
...
- [ ] ISC-80: The dashboard exposes a "Copy public status page link" action per monitor once published.
- [ ] ISC-81: `/dashboard` route redirects to `/login` when the session is missing or expired, rather than rendering an empty/broken page.
- [ ] ISC-82: The signup and login pages are reachable from an unauthenticated landing page at `/`.
...
- [ ] ISC-114: The app deploys successfully to Vercel and the deployed root URL returns 200.
...
### Operational
- [ ] ISC-119: `GET /api/health` returns 200 with a JSON body confirming DB connectivity, for external synthetic monitoring of the product itself.
