...
- 2026-09-01: Added an optional WAF-bypass shared-secret header to `runHttpCheck` (`http.ts`) — sends `x-uptime-monitor-key: <MONITOR_BYPASS_KEY>` on every HTTP check when that env var is set, unset by default so sites relying on the plain User-Agent (ISC-35) are unaffected. Chosen over UA-string tricks (e.g. impersonating a browser) specifically because a header keyed to a secret only this checker knows can't be spoofed by unrelated bot traffic the way a public, guessable UA string can — a target site's operator gets a real allowlist, not weakened bot protection. `MONITOR_BYPASS_KEY` set as a Vercel production env var (random 24-byte hex secret). Live-verified end-to-end via a throwaway monitor pointed at a `webhook.site` bin: the real deployed function's outbound request was captured with `x-uptime-monitor-key` present and matching the configured secret — proven against actual production traffic, not just local test coverage. Deploy for this required a second manual Deploy Hook trigger; the GitHub push webhook silently did not fire for this commit even though GitHub's HEAD was confirmed correct — same intermittent unreliability noted in the entry above, still unresolved.
- 2026-09-01: capital-cyber.com's Cloudflare 403 (see the two Decisions above) is still open as of this entry — the bypass-header mechanism is deployed and proven to work at the transport level, but the corresponding Cloudflare WAF rule (matching on the `x-uptime-monitor-key` header, action Skip → Bot Fight Mode/Managed Rules) has not yet been added on Anup's side. Once added, re-run the capital-cyber.com reproduction (throwaway signup + HTTP monitor + manual cron trigger) to confirm `statusCode` moves from 403 to 200.

- 2026-09-01: Vercel↔GitHub webhook auto-deploy started working again on its own for the next push (commit `c2e56dbd`, dashboard tabs + Help page) — no further intervention needed beyond what's described in the entry above. Cause of the earlier gap still unknown; noting it self-resolved rather than assuming it's permanently fixed.
- 2026-09-01: Investigated Anup's capital-cyber.com Cloudflare allowlist attempt (User-Agent-based rule) — still `statusCode: 403` after the change, reproduced live. Ruled out "Cloudflare blocks all bot traffic" as the explanation via a direct comparison: a PING monitor against capital-cyber.com shows `up` (expected — ping is a bare TCP handshake, never sends an HTTP request or User-Agent, so it can't trigger an HTTP-layer bot rule regardless of target), and an HTTP monitor against cybergrantsalliance.org (also Cloudflare-fronted) returns a clean `200`. So the block is specific to capital-cyber.com's own security configuration, not a platform-wide Cloudflare/Vercel-IP effect. Recommended checking Cloudflare's Security → Events log (not accessible from this environment) to identify the exact rule; not resolved as of this entry.
- 2026-09-01: Built MonitorTypeTabs + Help&Support Feature per Anup's request ("Add the differences to help and support public page. I want separate tabs for each features in the dashboard"). Scope was ambiguous on two axes, so locked it via `AskUserQuestion` before building (established pattern for this project) rather than guess: (1) Help page = full FAQ/support page (getting started, monitor-type differences, troubleshooting, alerts, status pages, account/billing) rather than just the check-type-differences content alone; (2) dashboard tabs = filter by monitor type (All/HTTP/Ping/TCP/Keyword/SSL), not by dashboard section. `codex` still absent from PATH (checked again), so per the same precedent as the ping/TCP session, wrote the code directly rather than spawning Forge to have it decline. Troubleshooting FAQ entries directly document the Ping-vs-HTTP asymmetry and the Cloudflare/WAF false-"down" scenario from the two Decisions above — this Feature exists partly because of exactly the confusion Anup and I worked through this session. `MonitorTabs` is a client component wrapping the existing `MonitorList` (no changes to `MonitorList` itself); filtering is client-side over the already-fetched monitor list, no new query. `bun run build` fails locally on unrelated pre-existing grounds (no local `DATABASE_URL`, confirmed via `git stash` that the same failure occurs on the pre-change tree too) — typecheck (clean) and the 71/71 test suite were the available local signals; full verification happened live post-deploy instead (Help page 200 + content present, dashboard tabs render all 6 labels for a real signed-in account with real monitors, confirmed via a throwaway signup — cleaned up after). Pushed as a single true atomic commit (`c2e56dbd`) via GitHub's Git Data API (blob→tree→commit→ref-update) rather than the Contents API loop used earlier this session, specifically to guarantee one commit regardless of file count — worth using this method by default going forward instead of relying on discipline to batch Contents-API calls correctly.
